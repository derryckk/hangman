...
var gameObject = {
	...
	wordsArray: ["APPLE", "ORANGE", "BANANA", "PEACH", "GRAPES", "MANGO", "PINEAPPLE", "WATERMELON", "KIWI"],
	randomWord: "",
	guessLetters:[],
	...
	generateWord: function(){
		...
		this.guessLetters = this.randomWord.split("");
	...
	checkMatch: function(){
		...
		for (var i=0; i < this.guessLetters.length; i++){
			if (this.currentLetter == this.guessLetters[i]){
				matchCounter++;
			}
		}
	...
	revealFruit: function(){
		...
		if (this.correctGuesses.length == 0){
			...
			for (var i=0; i<this.guessLetters.length; i++){
				...
				if (this.correctGuessesInOrder[i] != this.guessLetters[i]){
			
					for (var j=0; j<this.correctGuesses.length; j++){
			
						if (this.correctGuesses[j] == this.guessLetters[i]){
							this.correctGuessesInOrder[i] = this.guessLetters[i];
						}
	...
	checkProgress: function(){
		...
		if (counter == this.guessLetters.length){
			alert("You win");
			this.winCount++;
			this.generateWord();
		}
